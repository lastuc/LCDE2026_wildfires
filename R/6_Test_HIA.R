#-----------------------------------------------------------------------------#
#                 TEST Population-weighted exposures, RR and PAFs                 #
#-----------------------------------------------------------------------------#
...
# 1. RRs Anna Ailari: lag0: 1.008 (95% CI 1.006-1.010) per 1ug/m3 increment increase
set.seed(1234)
point_RR10 <- 1.008^10
nsim <- 200
...
# hia <- function(expofile, pRR10 = point_RR10, sRR10 = sim_RR10, mort = mortality){
  ...
  expodata <- read_csv(paste0(pathroot, "data/processed/assembled/data_2024.csv")) %>% 
  ...
  mort_nuts_year_T24 <- mort_nuts[, c(N = .N, lapply(.SD, sum)),
                              by = .(NUTS_mort, NUTS_0, region, eu, year),
                              .SDcols = attrcols]
  setDF(mort_nuts_year_T24)
  mort_nuts_year <- filter(mort_nuts_year_T24, N >= 365) %>%
    dplyr::select(-N) %>%
    complete(NUTS_mort, year)
  
  # Aggregate to countries ----
  mort_country_year_T24 <- mort_nuts_year_T24 %>%
    group_by(year, NUTS_0, region, eu) %>%
    summarise(across(starts_with("attr"), sum)) %>%
    ungroup()
  
  # Aggregate to regions ----
  mort_region_year_T24 <- mort_country_year_T24 %>%
  ...
  # Aggregate to EU ----
  mort_eu_year_T24 <- mort_country_year_T24 %>%
    group_by(year, eu) %>%
    summarise(across(starts_with("attr"), sum),
              Ncountries = n()) %>%
    ungroup()
  ...
  # Aggregate to Europe ----
  mort_euro_year_T24 <- mort_country_year_T24 %>%
    group_by(year) %>%
    summarise(across(starts_with("attr"), sum),
              Ncountries = n()) %>%
    ungroup()
  ...
  list("mortality_nuts" = mort_nuts_year,
       ...
       "mortality_euro" = mort_euro_year)->hia24
# }
...
attr_nuts <- bind_rows(#hia03[[1]]
                       # , hia04[[1]], hia05[[1]], hia06[[1]], hia07[[1]],
...
write_csv(attr_nuts, paste0(pathroot, "data/processed/attributable_nuts_2024.csv"))

# Attributable mortality by country
attr_country <- bind_rows(#hia03[[2]]
                          # , hia04[[2]], hia05[[2]], hia06[[2]], hia07[[2]],
...
write_csv(attr_country, paste0(pathroot, "data/processed/attributable_country_2024.csv"))

# Attributable mortality by region
attributable_region <- bind_rows(#hia03[[3]]
                                 # , hia04[[3]], hia05[[3]], hia06[[3]], hia07[[3]],
...
write_csv(attributable_region, paste0(pathroot, "data/processed/attributable_region_2024.csv"))

# Attributable mortality the EU
attr_eu <- bind_rows(#hia03[[4]]
                     # , hia04[[4]], hia05[[4]], hia06[[4]], hia07[[4]],
...
write_csv(attr_eu, paste0(pathroot, "data/processed/attributable_eu_2024.csv"))

# Attributable mortality euro
attr_euro <- bind_rows(#hia03[[5]]
                       # , hia04[[5]], hia05[[5]], hia06[[5]], hia07[[5]],
...
write_csv(attr_euro, paste0(pathroot, "data/processed/attributable_euro_2024.csv"))
