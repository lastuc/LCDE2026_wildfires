#-----------------------------------------------------------------------------#
#             HIA sensitivity analysis with RR from Alari lag0-7              #
#-----------------------------------------------------------------------------#
...
# 1. RRs Anna Ailari: lag0-7: 1.005 (95% CI 1.003-1.008) per 1ug/m3 increment increase
set.seed(1234)
point_RR10 <- 1.005^10
nsim <- 200
sim_RR10 <- exp(rnorm(nsim, mean = log(point_RR10), sd = log((1.008^10)/(1.003^10))/(qnorm(0.975)*2)))
...
hia <- function(expofile, pRR10 = point_RR10, sRR10 = sim_RR10, mort = mortality){
  ...
  mort_nuts_year_24 <- filter(mort_nuts_year, N >= 364) %>%
    dplyr::select(-N) %>%
...
attr_nuts <- bind_rows(hia03[[1]], hia04[[1]], hia05[[1]], hia06[[1]], hia07[[1]],
                       ...
                       hia23[[1]], hia24[[1]])
write_csv(attr_nuts, paste0(pathroot, "data/processed/attributable_nuts_sens_lag0-7.csv"))
...
attr_country <- bind_rows(hia03[[2]], hia04[[2]], hia05[[2]], hia06[[2]], hia07[[2]],
                          ...
                          hia23[[2]], hia24[[2]])
write_csv(attr_country, paste0(pathroot, "data/processed/attributable_country_sens_lag0-7.csv"))
...
attributable_region <- bind_rows(hia03[[3]], hia04[[3]], hia05[[3]], hia06[[3]], hia07[[3]],
                                 ...
                                 hia23[[3]], 
                                 hia24[[3]])
write_csv(attributable_region, paste0(pathroot, "data/processed/attributable_region_sens_lag0-7.csv"))
...
attr_eu <- bind_rows(hia03[[4]], hia04[[4]], hia05[[4]], hia06[[4]], hia07[[4]],
                     ...
                     hia23[[4]], hia24[[4]])
write_csv(attr_eu, paste0(pathroot, "data/processed/attributable_eu_sens_lag0-7.csv"))
...
attr_euro <- bind_rows(hia03[[5]], hia04[[5]], hia05[[5]], hia06[[5]], hia07[[5]],
                       ...
                       hia23[[5]], hia24[[5]])
write_csv(attr_euro, paste0(pathroot, "data/processed/attributable_euro_sens_lag0-7.csv"))
