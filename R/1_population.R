...
pop06_geom <-
  read_sf(paster0(pathroot, "data/raw/population/2006/Grid_ETRS89_LAEA_1K_ref_GEOSTAT_2006.shp"),
          quiet = T) |>
  rename(GRD_ID = GRD_INSPIR)
# Read population counts
pop06_counts <-
  read_delim(paster0(pathroot, "data/raw/population/2006/GEOSTAT_grid_EU_POP_2006_1K_V1_1_1.csv"),
             delim=";") |>
...
# Read polygon grid geometries
pop11_geom <-
  read_sf(paster0(pathroot, "data/raw/population/2011/GEOSTATReferenceGrid/Grid_ETRS89_LAEA_1K-ref_GEOSTAT_POP_2011_V2_0_1.shp"),
          quiet = TRUE)
# Read population counts, there may be duplicated if a cell is part of several countries
pop11_counts <- read_csv(paster0(pathroot, "data/raw/population/2011/GEOSTAT_grid_POP_1K_2011_V2_0_1.csv"))  |>
  dplyr::select(GRD_ID, TOT_P) |>
  rename(POP11 = TOT_P)
# Read modelled population counts
pop11_modcounts <- read_csv(paster0(pathroot, "data/raw/population/2011/JRC-GHSL_AIT-grid-POP_1K_2011.csv"))  |>
  dplyr::select(GRD_ID, TOT_P) |>
  rename(POP11 = TOT_P) |>
...
pop18 <- read_sf(paster0(pathroot, "data/raw/population/2018/JRC_POPULATION_2018.shp")) |>
...
# Read polygon grid geometries with population counts already included
pop21 <- st_read(paster0(pathroot, "data/raw/population/2021/ESTAT_Census_2021_V2.gpkg")) |>
...
gridgeom <- rast(paster0(pathroot, "data/raw/SILAM/europePMfire_2003to2024daymean.nc4"), lyrs = 1)
...
popall <- c(pop03_raster,pop04_raster,pop05_raster,
            ...
            pop21_raster,
            pop22_raster,pop23_raster,pop24_raster)
...
writeRaster(popall, "data/processed/population_yearly.tif",overwrite=T)
